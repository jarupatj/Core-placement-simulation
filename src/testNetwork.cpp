...
int main(int argc, char* argv[]) {
   ...
   network.changeConnection(from, to); //1-4
   to.x = 4;
   to.y = 3;
   network.addCore(to, 4); //core5
   network.changeConnection(from, to); //1-5
   ...
   to.x = 0;
   to.y = 1;
   network.changeConnection(from, to); //2-1
   to.x = 4;
   to.y = 3;
   network.changeConnection(from, to); //2-5
   ...
   to.x = 3;
   to.y = 0;
   network.changeConnection(from, to); //3-4

   from.x = 3;
   from.y = 0;
   to.x = 1;
   to.y = 4;
   network.changeConnection(from, to); //4-2
   to.x = 4;
   to.y = 6;
   network.changeConnection(from, to); //4-3

   from.x = 4;
   from.y = 3;
   to.x = 0;
   to.y = 1;
   network.changeConnection(from, to); //5-1
   to.x = 1;
   to.y = 4;
